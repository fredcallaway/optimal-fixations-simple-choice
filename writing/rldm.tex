...
\hypersetup{
  pdftitle={Attention in value-based choice as optimal sequential sampling}, pdfborder={0 0 0},
  breaklinks=true
}
...
\begin{abstract}
% Classical models of economic choice assume that when faced with a decision between several alternatives, people assign a value (utility) to each one and then choose the option with maximal value. However, a substantial body of evidence suggests that people do not determine exact utilities, but instead only approximate the (relative) values of each choice. Evidence accumulation models, such as the drift diffusion model (DDM), further postulate that the relative values are estimated sequentially, integrating a continuous stream of noisy evidence for or against each option. However, contrary to both intuition and behavioral evidence, such models typically assume that evidence for all options is accrued at a constant and uniform rate. A growing body of work, primarily using eye-tracking measures, has shown that attention plays a critical role in the decision making process. This suggests that the rate at which evidence for each option is accumulated is not constant, but is instead modulated by attentional processes. The recently proposed attentional drift diffusion model begins to address this drawback by introducing an attentional bias in the drift rate. However, this modification obscures the normative foundation of the DDM in statistical decision theory. Furthermore, the question of how attention is directed remains largely unexplored. To address these drawbacks, we cast attention allocation in decision making as a sequential sampling problem, in which an agent iteratively selects from which distribution to sample in order to update her beliefs about the values of the available alternatives. By approximating the optimal solution to this problem, we derive a model in which both the selection and integration of evidence are rational. This model predicts choices and reaction times, as well as visual fixations. Applying the model to a ternary-choice dataset, we find that its predictions align well with human data.

% Classical models of economic choice assume that when faced with a decision between several alternatives, people first assign a value to each one and then choose the option with maximal value. However, a substantial body of empirical evidence suggests that people do not determine exact utilities, but instead only approximate the (relative) values of each choice. Evidence accumulation models, such as the drift diffusion model (DDM), further postulate that the relative values are estimated sequentially by integrating a continuous stream of noisy evidence for or against each option. However, contrary to both intuition and behavioral evidence, such models typically assume that evidence for all the options is accrued at a constant and uniform rate. Recent work has demonstrated an important role of visual attention in the decision making process, suggesting that the rate at which evidence for each option is accumulated is not constant, but is instead modulated by attention. However, while the effects of attention on choice have been explored and modeled, the functional role attention plays in decision making remains poorly understood. In particular, despite strong evidence that attention is goal-directed, we are aware of no concrete model of how people choose to allocate their attention when making decisions. To address this gap, we cast attention allocation in decision making as a sequential sampling problem, in which an agent iteratively selects from which distribution to sample in order to update her beliefs about the values of the available alternatives. By approximating the optimal solution to this problem, we derive a model in which both the selection and integration of evidence are rational. This model predicts choices and reaction times, as well as visual fixations. Applying the model to a ternary-choice dataset, we find that its predictions align well with human data.

When faced with a decision between several options, people rarely fully consider every alternative. Instead, we direct our attention to the most promising candidates, focusing our limited cognitive resources on evaluating the options that we are most likely to choose. A growing body of empirical work has shown that attention plays an important role in human decision making, but it is still unclear how people choose with option to attend to at each moment in the decision making process. In this paper, we present an analysis of how a rational decision maker \emph{should} allocate her attention. We cast attention allocation in decision making as a sequential sampling problem, in which the decision maker iteratively selects from which distribution to sample in order to update her beliefs about the values of the available alternatives. By approximating the optimal solution to this problem, we derive a model in which both the selection and integration of evidence are rational. This model predicts choices and reaction times, as well as sequences of visual fixations. Applying the model to a ternary-choice dataset, we find that its predictions align well with human data.

% Despite a growing body of empirical work demonstrating the important role of attention in decision making, little is known about how people choose to allocate their attention when making decisions. 


\end{abstract}

\keywords{
decision-making; sampling; attention; bounded rationality;
}

\acknowledgements{This research was supported by a grant from Facebook Reality Labs.}
...
%Thus, we view the problem of how to allocate attention in decision making as a form of information search or active learning.
Importantly, the agent cannot simply allocate attention to the highest value item because she does not know the true values. Rather, she must decide which item to attend to based on her current value estimates. She may choose, for example, to focus her attention on items that she thinks have high value, or perhaps on items whose value is still highly uncertain. However, we do not specify how expected value and uncertainty should be traded off. Instead, we assume that the agent allocates attention in an (approximately) optimal way, where optimality is defined as maximizing decision utility minus computational cost.
 % The agent thus terminates the deliberation process when the cost of drawing more samples outweighs the potential benefit.

 % maximizing the quality of the final decision and minimizing the cost incurred by the decision making process.
...
% \end{equation}
% %
% which has the same expectation as the previous definition but lower variance.
% \citep{Hay2012}.
...
% Although, the parameters of the metalevel policy are optimized (reflecting our assumption that people allocate attention rationally), the meta-MDP itself has three free parameters that must be fit to human data. The sampling variance, $\sigma$, controls how quickly and reliably the estimates of each item's utility converges to to the true value. The cost parameters, $\samplecost$ and $\switchcost$, control the sampling cost and additional switching (saccade) cost. Ideally, we would optimize these parameters via maximum likelihood estimation. However, there is no analytical expression for the likelihood, and it is quite challenging to approximate it by simulation due to the high dimensionality of the data (in particular, the time series of fixations). Thus, we fit the model by minimizing the mean absolute error of a large set of summary statistics for a given dataset and model simulations.
...
\begin{figure}[t!]
  \centering
  \subfigure{
    \includegraphics[width=.3\textwidth]{figs/value_bias.pdf}
    \label{fig:value_bias}
  }
  \subfigure{
       \includegraphics[width=.3\textwidth]{figs/fixate_on_best.pdf}
       \label{fig:fixate_on_best}
  }
  \subfigure{
    \includegraphics[width=.3\textwidth]{figs/fixation_bias.pdf}
    \label{fig:value_bias}
  }
...
% Thus, we predict that people will be more likely to choose an item that they looked at more, accounting for any difference in the ratings previously assigned to each item.
